...
export default function RootLayout({
  ...
  // Verificar si estamos en páginas públicas o páginas de autenticación sin layout
  const publicRoutes = ['/', '/login', '/signup'];
  const noLayoutRoutes = ['/reset-password', '/confirm-email'];
  
  // Verificar si el path actual comienza con alguna de las rutas sin layout
  ...
  useEffect(() => {
    ...
    // Solo ejecutar si no estamos en una página pública o sin layout
    if (!isPublicPage && !isNoLayoutPage && isAuthenticated) {
      fetchUserProfile();
    }
  }, [pathname, user, isAuthenticated, isPublicPage, isNoLayoutPage]);
  ...
  return (
    <html lang="es">
      <body className="min-h-screen bg-gradient-to-br from-zinc-50/50 via-white/50 to-zinc-100/50 dark:from-[#0A0A0F] dark:via-[#12121A] dark:to-[#0A0A0F]">
        <ThemeProvider>
          {/* Si estamos en página sin layout, mostrar directamente el contenido */}
          {isNoLayoutPage ? (
            children
          ) : (
            <>
